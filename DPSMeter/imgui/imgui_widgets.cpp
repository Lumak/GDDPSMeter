...
void ImGui::GetCheckboxArray(const char* name, bool &checkbox)
{
  ImGuiContext& g = *GImGui;
  const ImGuiStyle& style = g.Style;
...
void ImGui::SetCheckboxArray(const char* name, bool checkboxes[], int arrSize)
{
  ...
  for (int i = 0; i < arrSize; ++i)
  {
    ...
    window->Checkbox[i] = checkboxes[i];
  }
}
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        //const bool is_cut   = ((is_shortcut_key && IsKeyPressed(ImGuiKey_X)) || (IsKeyPressed(ImGuiKey_Delete))) && !is_readonly && !is_password && (!is_multiline || state->HasSelection());
